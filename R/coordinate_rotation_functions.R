...
get.rotation.angle <- function(coord_df, group_var, degree = TRUE){
	if(length(unique(coord_df[[group_var]])) > 2) {
		print("This function only works for two groups")
	} else {
		centr_df <- data.frame(coord_df %>% group_by(get(group_var)) %>% summarise(across(c("dim1", "dim2"), mean)))
...
rotate.coords <- function(coord_df, group_var, col_names = c("dim1", "dim2"), degree = TRUE){
	angle <- get.rotation.angle(coord_df, group_var, degree = degree)
	# why negative?!
